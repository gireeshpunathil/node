...
#endif
// Include execinfo.h for the native backtrace API. The API is
// unavailable on AIX and on some Linux distributions, e.g. Alpine Linux.
...
static void WriteNodeReport(Isolate* isolate, DumpEvent event,
                            const char* message, const char* location,
                            char* filename, std::ostream &out,
                            MaybeLocal<Value> error, TIME_TYPE* time);
static void PrintCommandLine(std::ostream& out);
static void PrintVersionInformation(std::ostream& out);
static void PrintJavaScriptStack(std::ostream& out, Isolate* isolate,
                                 DumpEvent event, const char* location);
static void PrintJavaScriptErrorStack(std::ostream& out, Isolate* isolate,
                                      MaybeLocal<Value> error);
static void PrintStackFromStackTrace(std::ostream& out, Isolate* isolate,
                                     DumpEvent event);
static void PrintStackFrame(std::ostream& out, Isolate* isolate,
                            Local<StackFrame> frame, int index, void* pc);
...
char report_directory[NR_MAXPATH + 1] = "";  // default: current directory
TIME_TYPE loadtime_tm_struct;  // module load time
time_t load_time;  // module load time absolute

// std::string version_string = NODE_VERSION_STRING;
// std::string commandline_string = "";
version_and_command_struct version_and_command;
...
void TriggerNodeReport(Isolate* isolate, DumpEvent event, const char* message,
  ...
  if (!strncmp(filename, "stdout", sizeof("stdout") - 1)) {
    ...
    if (strlen(report_directory) > 0) {
      ...
      snprintf(pathname, sizeof(pathname),
               "%s%s%s", report_directory, "\\", filename);
#else
      snprintf(pathname, sizeof(pathname),
               "%s%s%s", report_directory, "/", filename);
#endif
    ...
    if (!outfile.is_open()) {
      if (strlen(report_directory) > 0) {
        std::cerr << "\nFailed to open Node.js report file: "
                  << filename << " directory: " << report_directory
                  << " (errno: " << errno << ")\n";
      } else {
        std::cerr << "\nFailed to open Node.js report file: "
  ...
  WriteNodeReport(isolate, event, message,
                  location, filename, out, error, &tm_struct);
  ...
  if (name != nullptr) {
    snprintf(name, NR_MAXNAME + 1, "%s", filename);  // return report file name
  }
}
 ...
 *
 *****************************************************************************/
void GetNodeReport(Isolate* isolate, DumpEvent event, const char* message,
                   const char* location, MaybeLocal<Value> error,
                   std::ostream& out) {
  // Obtain the current time and the pid (platform dependent)
  TIME_TYPE tm_struct;
#ifdef _WIN32
...
#endif
  WriteNodeReport(isolate, event, message,
                  location, nullptr, out, error, &tm_struct);
}
...
static void WriteNodeReport(Isolate* isolate, DumpEvent event,
                            const char* message, const char* location,
                            char* filename, std::ostream &out,
                            MaybeLocal<Value> error, TIME_TYPE* tm_struct) {
#ifdef _WIN32
  DWORD pid = GetCurrentProcessId();
#else  // UNIX, OSX
  pid_t pid = getpid();
  ...
  snprintf(timebuf,
           ...
           loadtime_tm_struct.wYear,
           loadtime_tm_struct.wMonth,
           loadtime_tm_struct.wDay,
           loadtime_tm_struct.wHour,
           loadtime_tm_struct.wMinute,
  ...
  snprintf(timebuf,
           sizeof(timebuf),
           "%4d/%02d/%02d %02d:%02d:%02d",
           loadtime_tm_struct.tm_year+1900,
           loadtime_tm_struct.tm_mon+1,
           loadtime_tm_struct.tm_mday,
           loadtime_tm_struct.tm_hour,
           loadtime_tm_struct.tm_min,
...
// Print summary JavaScript stack backtrace
  ...
  out << std::left << std::setw(7) << "Flags" << std::setw(10) << "Type"
      ...
      << std::endl;
  uv_walk(uv_default_loop(), walkHandle, reinterpret_cast<void*>(&out));
 ...
 ******************************************************************************/
static void PrintCommandLine(std::ostream& out) {
  if (version_and_command.commandline_string != "") {
    out << "Command line: " << version_and_command.commandline_string << "\n";
  }
}
...
static void PrintVersionInformation(std::ostream& out) {
  ...
  out << "\n" << version_and_command.version_string;

  // Print Node version
  out << std::endl << "Node.js v" <<  NODE_VERSION_STRING;
...
#ifdef _WIN32
  {
    ...
    NET_API_STATUS nStatus = NetServerGetInfo(nullptr, level,
                               reinterpret_cast<LPBYTE *>(&os_info));
    if (nStatus == NERR_Success) {
      ...
      // Convert and print the machine name and comment fields
      // (these are LPWSTR types)
      size_t count;
      char name_buf[256];
      wcstombs_s(&count, name_buf, sizeof(name_buf),
                 os_info->sv101_name, _TRUNCATE);
  ...
  if (uname(&os_info) >= 0) {
    ...
    *(reinterpret_cast<void**>(&libc_version)) = dlsym(RTLD_DEFAULT,
...
static void PrintJavaScriptStack(std::ostream& out, Isolate* isolate,
  ...
  case kFatalError:
    ...
    break;
  default:
    // All other events, print the stack using
    // StackTrace::StackTrace() and GetStackSample() APIs
...
static void PrintJavaScriptErrorStack(std::ostream& out, Isolate* isolate,
  ...
  out << "\n==== JavaScript Exception Details ====="
         "=========================================\n\n";
  Local<Message> message = Exception::CreateMessage(isolate,
                                                    error.ToLocalChecked());

  out << *message << "\n\n";

  Local<StackTrace> stack = Exception::GetStackTrace(error.ToLocalChecked());
  if (stack.IsEmpty()) {
     out << "No stack trace available from Exception::GetStackTrace()\n";
     return;
  }
  // Print the stack trace,
  // samples are not available as the exception isn't from the current stack.
 ...
 *
 ******************************************************************************/
static void PrintStackFromStackTrace(std::ostream& out,
                                     Isolate* isolate, DumpEvent event) {
  RegisterState state;
  SampleInfo info;
  void* samples[255];
  ...
  // Print the stack trace, adding in the
  // pc values from GetStackSample() if available
  for (int i = 0; i < stack->GetFrameCount(); i++) {
 ...
 ******************************************************************************/
static void PrintStackFrame(std::ostream& out, Isolate* isolate,
                            Local<StackFrame> frame, int i, void* pc) {
  ...
  for (int i = 0; i < numberOfFrames; i++) {
    ...
    if (SymFromAddr(hProcess, dwAddress, &dwOffset64, pSymbol)) {
      ...
      snprintf(buf, sizeof(buf), "%2d: [pc=0x%p]\n", i,
               reinterpret_cast<void*>(dwAddress));
      out << buf;
    }
  }
}
...
void PrintNativeStack(std::ostream& out) {
  ...
  // Print the native frames, omitting the top 3 frames as they are
  // in node-report code backtrace_symbols_fd(frames, size, fileno(fp));
  for (int i = 2; i < size; i++) {
    // print frame index and instruction address
    snprintf(buf, sizeof(buf), "%2d: [pc=%p] ", i-2, frames[i]);
    out << buf;
    // If we can translate the address using dladdr() print
...
static void PrintResourceUsage(std::ostream& out) {
  ...
  if (getrusage(RUSAGE_SELF, &stats) == 0) {
    ...
    snprintf(buf, sizeof(buf), "%ld.%06d",
 ...
 ******************************************************************************/
#ifdef __linux__
static int LibraryPrintCallback(struct dl_phdr_info* info,
...
static void PrintLoadedLibraries(std::ostream& out, Isolate* isolate) {
  ...
  if (EnumProcessModules(process_handle, nullptr, 0, &size_1)) {
    HMODULE* modules = reinterpret_cast<HMODULE*>(malloc(size_1));
    if (modules == null) {
