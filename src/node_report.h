...
typedef struct version_and_command_struct {
  std::string version_string = NODE_VERSION_STRING;
  std::string commandline_string = "";
}version_and_command_struct;
extern version_and_command_struct version_and_command;
...
void TriggerNodeReport(Isolate* isolate, DumpEvent event,
                       ...
                       char* name, v8::MaybeLocal<v8::Value> error);
void GetNodeReport(Isolate* isolate, DumpEvent event,
                   const char* message, const char* location,
                   v8::MaybeLocal<v8::Value> error, std::ostream& out);
