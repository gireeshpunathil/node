...
unsigned int ProcessNodeReportEvents(const char* args) {
  ...
  while (*cursor != '\0') {
    if (!strncmp(cursor, "exception", sizeof("exception") - 1)) {
      ...
      std::cerr << "Unrecognised argument for node-report events option: "
                << cursor
                << "\n";
      return 0;
    }
...
unsigned int ProcessNodeReportSignal(const char* args) {
  ...
  if (strlen(args) == 0) {
    ...
    if (!strncmp(args, "SIGUSR2", sizeof("SIGUSR2") - 1)) {
     ...
     std::cerr << "Unrecognised argument for node-report signal option:"
               << args
               << "\n";
    }
  }
...
void ProcessNodeReportFileName(const char* args) {
  ...
  if (strlen(args) > NR_MAXNAME) {
    std::cerr << "Supplied node-report filename too long (max "
              << NR_MAXNAME
              << " characters)\n";
    return;
  }
...
void ProcessNodeReportDirectory(const char* args) {
  ...
  if (strlen(args) > NR_MAXPATH) {
    std::cerr << "Supplied node-report directory path too long (max "
              << NR_MAXPATH
              << " characters)\n";
    return;
  }
...
unsigned int ProcessNodeReportVerboseSwitch(const char* args) {
  ...
  // Parse the supplied switch
  if (!strncmp(args, "yes", sizeof("yes") - 1) ||
      !strncmp(args, "true", sizeof("true") - 1)) {
    return 1;
  } else if (!strncmp(args, "no", sizeof("no") - 1) ||
             !strncmp(args, "false", sizeof("false") - 1)) {
    return 0;
  } else {
    std::cerr << "Unrecognised argument for node-report verbose switch option: "
              << args
              << "\n";
  }
...
void SetVersionString(Isolate* isolate) {
  ...
  version_and_command.version_string = NODE_VERSION_STRING;

  // Retrieve the process object
  Local<String> process_prop = String::NewFromUtf8(
                                 Isolate::GetCurrent(),
                                 "process",
                                 NewStringType::kNormal)
                                 .ToLocalChecked();
  if (process_prop->IsNull()) return;

  Local<Object> global_obj = isolate->GetCurrentContext()->Global();
  EscapableHandleScope scope(isolate);
  Local<Value> process_value = scope.Escape(
                                         global_obj->Get(
                                         isolate->GetCurrentContext(),
                                         process_prop)
                                         .FromMaybe(Local<Value>()));
  if (process_value->IsNull()) return;

  if (!process_value->IsObject()) return;
  Local<Object> process_obj = process_value.As<Object>();

  // Get process.versions
  Local<String> versions_prop = String::NewFromUtf8(isolate,
                                                    "versions",
                                                    NewStringType::kNormal)
                                                    .ToLocalChecked();
  if (versions_prop->IsNull()) return;

  Local<Value> versions_value = process_obj->Get(isolate->GetCurrentContext(),
                                                 versions_prop)
                                                 .ToLocalChecked();
  if (versions_value->IsNull()) return;
  if (!versions_value->IsObject()) return;
  Local<Object> versions_obj = versions_value.As<Object>();

  // Get component names and versions from process.versions
  Local<Array> components = versions_obj->GetOwnPropertyNames(
                              isolate->GetCurrentContext())
                              .FromMaybe(Local<Array>());
  if (components->IsNull()) return;

  Local<Object> components_obj = components.As<Object>();
  std::string comp_versions = "(";
  size_t wrap = 0;
  uint32_t total_components = (*components)->Length();
  for (uint32_t i = 0; i < total_components; i++) {
    Local<Value> name_value = components_obj->Get(isolate->GetCurrentContext(),
                                                  i)
                                                  .ToLocalChecked();
    if (name_value->IsNull()) continue;
    Local<Value> version_value = versions_obj->Get(isolate->GetCurrentContext(),
                                                   name_value)
                                                   .ToLocalChecked();
    if (version_value->IsNull()) continue;

    const String::Utf8Value component_name(isolate, name_value);
    String::Utf8Value component_version(isolate, version_value->ToString());
    ...
    if (!strcmp("node", *component_name)) {
      ...
      if (version_and_command.version_string == NODE_VERSION_STRING) {
        version_and_command.version_string = "Node.js version: v";
        version_and_command.version_string += *component_version;
        version_and_command.version_string += "\n";
      }
    } else {
      // Other component versions follow,
      // comma separated, wrapped at 80 characters
  ...
  version_and_command.version_string += comp_versions + ")\n";
}
...
void SetCommandLine() {
  ...
  version_and_command.commandline_string = "";
  ...
  while (bytesread > 0) {
    for (int i = 0; i < bytesread; i++) {
      ...
      if (buf[i] == '\0') {
        ...
        version_and_command.commandline_string += buf[i];
      }
    }
  ...
  for (int i = 0; i < argc; i++) {
    version_and_command.commandline_string += separator + argv[i];
    separator = " ";
  }
  ...
  if (bytesread == sizeof(psinfo_t)) {
    ...
    std::string separator = "";
#ifdef _AIX
    char **argv = *(reinterpret_cast<char ***>(info.pr_argv));
#else
    char **argv = (reinterpret_cast<char ***>(info.pr_argv));
#endif
...
#elif _WIN32
  version_and_command.commandline_string = GetCommandLine();
#endif
}
