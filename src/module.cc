...
void TriggerReport(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (info[0]->IsString()) {
    ...
    v8::String::Utf8Value filename_parameter(isolate, info[0]);
 ...
 *
 ******************************************************************************/
void SetEvents(const v8::FunctionCallbackInfo<v8::Value>& info) {
  v8::Isolate* isolate = info.GetIsolate();
  v8::String::Utf8Value parameter(isolate, info[0]);
...
void SetSignal(const v8::FunctionCallbackInfo<v8::Value>& info) {
#ifndef _WIN32
  v8::String::Utf8Value parameter(info.GetIsolate(), info[0]);
...
void SetFileName(const v8::FunctionCallbackInfo<v8::Value>& info) {
  v8::String::Utf8Value parameter(info.GetIsolate(), info[0]);
  ProcessNodeReportFileName(*parameter);
}
void SetDirectory(const v8::FunctionCallbackInfo<v8::Value>& info) {
  v8::String::Utf8Value parameter(info.GetIsolate(), info[0]);
  ProcessNodeReportDirectory(*parameter);
}
void SetVerbose(const v8::FunctionCallbackInfo<v8::Value>& info) {
  v8::String::Utf8Value parameter(info.GetIsolate(), info[0]);
...
static void PrintStackFromStackTrace(Isolate* isolate, FILE* fp) {
  ...
  for (int i = 0; i < stack->GetFrameCount(); i++) {
    ...
    v8::String::Utf8Value fn_name_s(isolate, frame->GetFunctionName());
...
static void SetupSignalHandler() {
  ...
  if (rc != 0) {
    ...
    isolate->ThrowException(v8::String::NewFromUtf8(
                            isolate,
                            "node-report: initialization failed, uv_sem_init() returned error\n",
                            NewStringType::kNormal).ToLocalChecked());
  ...
  if (rc != 0) {
    ...
    isolate->ThrowException(v8::String::NewFromUtf8(
                            isolate,
                            "node-report: initialization failed, uv_mutex_init() returned error\n",
                            NewStringType::kNormal).ToLocalChecked());
  }
  ...
  if (StartWatchdogThread(ReportSignalThreadMain) == 0) {
    ...
    isolate->ThrowException(v8::String::NewFromUtf8(
                            isolate,
                            "node-report: initialization failed, uv_async_init() returned error\n",
                            NewStringType::kNormal).ToLocalChecked());
    }
