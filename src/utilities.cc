...
void SetVersionString(Isolate* isolate) {
  ...
  v8::Local<v8::String> process_prop =  v8::String::NewFromUtf8(v8::Isolate::GetCurrent(),
                                                                "process",
  ...
  v8::Local<v8::Value> process_value = scope.Escape(global_obj->Get(
  ...
  // Get process.version
  v8::Local<v8::String> version_prop = v8::String::NewFromUtf8(isolate,
                                                               "version",
                                                               v8::NewStringType::kNormal).ToLocalChecked();
  if(version_prop->IsNull()) return;
  v8::Local<v8::Value> version = process_obj->Get(isolate->GetCurrentContext(),
                                                  version_prop)
                                                  .ToLocalChecked();
  ...
  if (version->IsString()) {
    const v8::String::Utf8Value node_version(isolate, version);
  ...
  v8::Local<v8::String> versions_prop = v8::String::NewFromUtf8(isolate,
                                                                "versions",
                                                                v8::NewStringType::kNormal)
                                                                .ToLocalChecked();
  if (versions_prop->IsNull()) return;

  v8::Local<v8::Value> versions_value = process_obj->Get(isolate->GetCurrentContext(),
                                                         versions_prop)
                                                         .ToLocalChecked();
  ...
  v8::Local<v8::Array> components = versions_obj->GetOwnPropertyNames(
                                        isolate->GetCurrentContext()).FromMaybe(v8::Local<v8::Array>());
  ...
  for (uint32_t i = 0; i < total_components; i++) {
    v8::Local<v8::Value> name_value = components_obj->Get(isolate->GetCurrentContext(),
                                                          i)
                                                          .ToLocalChecked();
    if (name_value->IsNull()) continue;
    v8::Local<v8::Value> version_value = versions_obj->Get(isolate->GetCurrentContext(),
                                                           name_value)
                                                           .ToLocalChecked();
    if (version_value->IsNull()) continue;

    const v8::String::Utf8Value component_name(isolate, name_value);
    v8::String::Utf8Value component_version(isolate, version_value->ToString());
